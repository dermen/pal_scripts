...
class HistUpdater(tk.Frame):
    ...
    def __init__(self, parent, data, fig, ax, color="blue", label="label", 
            max_bins=1000):
        ...
        ypos = y1# - (y1-y0)*.1
        self.fig.text((x0+x1)*.5, ypos, label, backgroundcolor='w', color='k', 
...
if __name__ == "__main__":
    ...
    for i_row in xrange( nrows):
        for i_col in xrange( ncols):
            ...
            dat = DAT[i_ax]
            color = colors[i_ax]
            HistUpdater(root, dat, fig, ax, color=color, label=label).pack(fill="both", expand=True)
            i_ax +=1
    plt.draw()
    plt.pause(0.0001)
